...
class Match:
    def __init__(self, view1: View, view2: View) -> None:
        ...
        matches_path = self.dataset_path / "matches"
        if not matches_path.exists():
            matches_path.mkdir()
        if not (path := matches_path / f"{self.image_name1}-{self.image_name2}.pkl").exists():
            if match_technique == "SIFT":
                self.get_matches_SIFT()
            elif match_technique == "LoFTR":
                self.get_matches()
            # print(f"=========Done matching {self.image_name1} and {self.image_name2}=========")
            self.store_data(path)
    ...
    def get_matches_SIFT(self):
        self.matches = self.matcher_SIFT.match(
            trainDescriptors=self.view1.descriptors,
            queryDescriptors=self.view2.descriptors,
        )
        ...
        if len(self.pixel_points1) > 7:
            self.F, self.mask = cv2.findFundamentalMat(
                np.array(self.pixel_points1)[self.indices1],
                np.array(self.pixel_points2)[self.indices2],
        ...
        for i in range(len(self.pixel_points1)):
            self.matches.append(
                Match_info(
                    ...
                    self.indices2[i],
                    self.confidence[i],
                    self.pixel_points1[i],
                    self.pixel_points2[i],
                )
            )
    ...
    def store_data(self, path: Path) -> None:
        data = [
            self.F,
            self.E,
            self.mask,
    ...
    def draw_matches(self) -> None:
        ...
        concatImg = np.zeros(
            (
                max(self.view1.scaled_image.shape[0], self.view2.scaled_image.shape[0]),
                self.view1.scaled_image.shape[1] + self.view2.scaled_image.shape[1],
                3,
            ),
            dtype=np.uint8,
        )
        ...
        cv2.imwrite(
            os.path.join(
                self.dataset_path,
                "matches",
                f"{self.image_name1}-{self.image_name2}.png",
            ),
            concatImg,
        )
